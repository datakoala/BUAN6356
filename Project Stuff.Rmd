...
levels(ab.dt$city)<- c("NC", "TX", "MA", "FL", "MA", "IL", "NV", "OH", "CO", "HI", "NJ", "CA", "TN", "LA", "NY", "CA", "CA", "OR", "RI", "OR", "CA", "CA", "CA", "CA", "CA", "WA", "MN", "D.C.")
...
```{r correlation plot}
corr.Ab <- ab.dt[,c(-2,-4,-6,-10,-11,-14)]
...
pcs <- prcomp(data.table(ab.dt$price, ab.dt$reviews_per_month)) 
summary(pcs) #need only numeric variables for pc analysis???
...
# Visualize using Screeplot
fviz_eig(pcs.cor, barfill = "bisque", barcolor = "NA", linecolor = "blue")
```
...
##Ridge Regression
```{r}
ridgeReg <- train(price~., train.df, method = 'glmnet',
...
##Lasso Regression
```{r}
lassoReg <- train(price~., train.df, method = 'glmnet',
...
```{r}
enetReg <- train(price~., train.df, method = 'glmnet',
...
```{r}
ab1.lm.bselect <- step(ab1.lm, direction = "backward")
summary(ab1.lm.bselect)  # Which variables were dropped?
ab1.lm.bselect.pred <- predict(ab1.lm.bselect, valid.df)
accuracy(ab1.lm.bselect.pred, valid.df$price)
```

##Forward Regression
```{r}
# create model with no predictors
ab1.lm.null <- lm(price~1, data = train.df)

# use step() to run forward regression.
ab1.lm.fselect <- step(ab1.lm.null, scope=list(lower=ab1.lm.null, upper=ab1.lm), direction = "forward")
summary(ab1.lm.fselect)  # Which variables were added?
ab1.lm.fselect.pred <- predict(ab1.lm.fselect, valid.df)
accuracy(ab1.lm.fselect.pred, valid.df$price)
```
...
set.seed(123)
sample<- sample.int(n=nrow(ab.log),size=floor(round(0.80*nrow(ab.log))), replace = F)
