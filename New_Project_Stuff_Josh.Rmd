...
pacman::p_load(esquisse, forecast, tidyverse, 
               ...
               mosaic, scales, mapproj, mlbench, data.table, ggcorrplot, leaps, corrplot, MASS, caret, ggplot2, knitr, ggmap, grid, mlbench, gridExtra,ggplot2,lattice,ROSE,randomForest,dplyr,e1071,rpart,rpart.plot,mosaicData,readr,reshape, factoextra, pls)
...
```{r Read In Data}
  ab.df <- fread("AB_US_2020_Cleaned.csv")
...
ggplot(ab.dt) +
 aes(x = price, y = nrow(ab.dt)) +
 geom_violin(scale = "area", fill = "#fee0d2", trim = FALSE) + 
  geom_boxplot(fill = "rosybrown2", outlier.color = "seashell3", width = 0.1) +
  ggtitle("Violin Plot")
```
...
ggplot(ab.dt) +
 aes(x = reviews_per_month, y = nrow(ab.dt)) +
 geom_violin(scale = "area", fill = "#fee0d2", trim = FALSE) + 
  geom_boxplot(fill = "rosybrown2", outlier.color = "seashell3", width = 0.1) +
  ggtitle("Violin Plot")
```
...
ggplot(ab.dt) +
 aes(x = availability_365, y = nrow(ab.dt)) +
 geom_violin(scale = "area", fill = "blue", trim = FALSE) + 
  geom_boxplot(fill = "red", outlier.color = "seashell3", width = 0.1) +
  ggtitle("Violin Plot")
```
...
##Dimension Reduction
```{r}
  ### compute PCs on all quantitative variables in datatable
pcsfact <- FactoMineR::PCA(ab.dt[, -c(2, 6, 10, 14)])
summary(pcsfact)

  ### PCA on Quantitative Variables using code learned in class creating datafram from our datatable as pca with this code is only on a dataframe
pcs.df <- as.data.frame(ab.dt)
pcsvar <- prcomp(na.omit(pcs.df[,-c(2, 6, 10, 14)])) 
summary(pcsvar)
pcsvar$rot

  ### PCA using Normalized variables using code learned in class
pcs.cor <- prcomp(na.omit(pcs.df[,-c(2, 6, 10, 14)]), scale. = T)
summary(pcs.cor)
pcs.cor$rot

# Visualize using Screeplot comparing two different pca techniques
pcsclass <- fviz_eig(pcs.cor, barfill = "bisque", barcolor = "NA", linecolor = "blue", title = "PCS Class")

pcsclass

factpcs <- fviz_eig(pcsfact, barfill = "bisque", barcolor= "NA", linecolor = "blue", title = "FactoMineR")

factpcs

##There is no difference in these two lines of pca code, just depends on which one we want to use
```
...
#fit PCR model
model <- pcr(price~., data=pcs.df, scale=TRUE, validation="CV")

##Won't let me run this regression because I don't have enough memory apparently
```
